...
int main(int argc, char* argv[]) {
  try {
    ...
    const auto* env_num_threads = std::getenv("TBB_NUM_THREADS");
    if (env_num_threads != nullptr) {
      num_threads = static_cast<std::size_t>(std::max(1, std::atoi(env_num_threads)));
    }
    tbb::global_control gc_num_threads{tbb::global_control::max_allowed_parallelism, num_threads};
    std::cout << "num_threads: "
              << tbb::global_control::active_value(tbb::global_control::max_allowed_parallelism)
              << " (can be set by TBB_NUM_THREADS)" << std::endl;
