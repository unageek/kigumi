...
template <class K, class FaceData>
CGAL::Bbox_3 face_bbox(const Triangle_soup<K, FaceData>& m, Face_handle handle) {
  const auto& f = m.face(handle);
  return m.point(f[0]).bbox() + m.point(f[1]).bbox() + m.point(f[2]).bbox();
}

template <class K, class FaceData>
typename K::Point_3 face_centroid(const Triangle_soup<K, FaceData>& m, Face_handle handle) {
  const auto& f = m.face(handle);
  return CGAL::centroid(m.point(f[0]), m.point(f[1]), m.point(f[2]));
}

template <class K, class FaceData>
typename K::Point_3 face_centroid(const Triangle_mesh<K, FaceData>& m, Face_handle handle) {
  const auto& f = m.face(handle);
  return CGAL::centroid(m.point(f[0]), m.point(f[1]), m.point(f[2]));
}
